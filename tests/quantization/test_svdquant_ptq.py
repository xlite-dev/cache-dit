...
def _resolve_saved_flux2_checkpoint_dir(case_name: str, plan: Flux2ExecutionPlan) -> Path:
    ...
    if not quant_config_path.is_file():
        pytest.skip(
            "Saved FLUX2 quantized checkpoint not found. Run "
    ...
    *,
    ...
    if any(image is None for image in images):
        raise FileNotFoundError(
            f"Failed to load FLUX2 comparison images for prompt_{prompt_index}: "
    ...
    cv2.putText(
        canvas, subtitle, (12, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (40, 40, 40), 1, cv2.LINE_AA
    )
    return canvas
    ...
    *,
    ...
    for attempt, plan in enumerate(_flux2_execution_plans(), start=1):
        ...
        try:
            ...
            loaded_pipe = Flux2KleinPipeline.from_pretrained(
                model_source, torch_dtype=torch.bfloat16
            )
            ...
            assert (
                tuple(getattr(loaded_pipe.transformer, "_svdq_quantized_layers", ()))
            ...
            report_paths = {
                "execution_plan": _emit_flux2_report(
                ...
                "consistency": _emit_flux2_report(
                    report_dir=reports_dir,
                ...
                "acc_metrics": _emit_flux2_report(
                    report_dir=reports_dir,
...
def test_svdq_ptq_public_api_flush_thresholds_preserve_quantized_outputs(tmp_path: Path) -> None:
    ...
    assert tuple(getattr(buffered_quantized, "_svdq_quantized_layers", ())) == tuple(
        getattr(baseline_quantized, "_svdq_quantized_layers", ())
    )
...
def test_svdq_ptq_load_rejects_invalid_or_incomplete_metadata(tmp_path: Path) -> None:
    ...
    save_file(
        ...
        metadata={
            _SVDQ_METADATA_KEY_NAME: '{"format":"cache_dit_svdq_ptq","version":1,"quant_type":"svdq_int4_r32","rank":32,"quantized_layer_names":["block.to_q"],"svdq_kwargs":{"streaming":true,"high_precision":false,"fp32_fallback":true}}'
        },
    )
    with pytest.raises(ValueError, match="No serialized tensors found"):
        cache_dit.load(
            make_toy_model(
                embed_dim=128, num_heads=4, seed=73, device="cuda", dtype=runtime_dtype()
            ),
            str(incomplete_metadata_path),
        )
...
def test_svdq_ptq_flux2_klein_loaded_quantized_transformer_uses_less_cuda_memory() -> None:
    ...
    try:
        ...
        float_pipe.to("cuda")
        float_transformer_cuda_gib = _compute_module_cuda_tensor_bytes(float_pipe.transformer) / (
            1024**3
        )
...
@pytest.mark.skipif(
    ...
    reason=(
        "FLUX.2 SVDQ PTQ compile integration test requires "
