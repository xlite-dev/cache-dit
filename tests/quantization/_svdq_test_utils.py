...
def make_spectral_decay_weight(
  ...
  *,
  ...
  left = torch.randn(out_features, rank, generator=generator, dtype=torch.float32).to(device=device)
  right = torch.randn(in_features, rank, generator=generator, dtype=torch.float32).to(device=device)
...
def collect_module_inputs(
  ...
  for module_name in module_names:
    ...
    def hook(_module: nn.Module, args: tuple[torch.Tensor, ...], name: str = module_name) -> None:
