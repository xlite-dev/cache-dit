// Copied from nunchaku/src/interop/torch.cpp
#include "torch.h"
...
Tensor from_torch(at::Tensor input) {
  ...
  for (int i = 0; i < ndims; i++) {
    result.shape.dataExtent.push_back(
      int_cast<decltype(result.shape.dataExtent)::value_type>(input.size(i)));
    result.shape.dataStride.push_back(
      int_cast<decltype(result.shape.dataStride)::value_type>(input.stride(i)));
  }
