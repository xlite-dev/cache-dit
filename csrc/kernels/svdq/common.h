// Copied from nunchaku/src/common.h
#pragma once
...
class CUDAError : public std::runtime_error {
 ...
 private:
  static std::string format(cudaError_t errorCode, std::source_location location) {
    return spdlog::fmt_lib::format("CUDA error: {} (at {}:{})", cudaGetErrorString(errorCode),
...
inline cublasStatus_t checkCUBLAS(
  ...
  if (retValue != CUBLAS_STATUS_SUCCESS) {
    throw std::runtime_error(spdlog::fmt_lib::format("CUBLAS error: {} (at {}:{})",
                                                     cublasGetStatusString(retValue),
                                                     location.file_name(), location.line()));
  }
  return retValue;
}
...
struct CUDAStreamContext {
  ...
  CUDAStreamContext(cudaStream_t stream) : stream(stream) { stackCUDAStreams.push(stream); }
...
struct CUDAStreamWrapper {
  ...
  CUDAStreamWrapper() { checkCUDA(cudaStreamCreate(&stream)); }
  CUDAStreamWrapper(const CUDAStreamWrapper &) = delete;
  CUDAStreamWrapper(CUDAStreamWrapper &&) = delete;

  ~CUDAStreamWrapper() { checkCUDA(cudaStreamDestroy(stream)); }
};
...
struct CUDAEventWrapper {
  ...
  ~CUDAEventWrapper() { checkCUDA(cudaEventDestroy(event)); }
};
...
class CUDADeviceContext {
 public:
  ...
  ~CUDADeviceContext() {
    ...
    if (cacheDisabled()) {
      ...
      // otherwise => we are a nested context in a previous context with reset on, we might continue
      // to execute external code, reset
      currentDeviceCache = -1;
    }
  }
 ...
 private:
  ...
  static bool cacheDisabled() { return ctxs.empty() || ctxs.top()->disableCache; }
};
...
struct CUBLASWrapper {
  ...
  CUBLASWrapper() { checkCUBLAS(cublasCreate(&handle)); }
