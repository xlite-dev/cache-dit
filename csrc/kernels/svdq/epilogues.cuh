// Copied from nunchaku/src/kernels/zgemm/epilogues.cuh
#pragma once
...
#pragma unroll
  ...
  struct EpilogueQKVProj {
    ...
    __device__ __forceinline__ static void apply(fpsum_warp fpsum, half_t *out, int M, int N, int K,
                                                 half_t *pool_out, const float *rotary_emb,
                                                 const half_t *rmsnorm_weight, float epsilon,
                                                 int maxRows) {
      ...
      const float *rotary_emb_base_addr =
        &rotary_emb[(warpId * WARP_M) * HEAD_DIM / 2 * ROTARY_EMB_NUM_ELEMENTS +
                    laneId * PACK_SIZE / 2 * ROTARY_EMB_NUM_ELEMENTS];

      CHECK_NAN(fpsum, "fpsum");

      unpack_fpsum()(fpsum, out + warpId * WARP_M * N, N, maxRows - warpId * WARP_M, INT_MAX,
                     shmem[warpId], [&](int rowId, pack_t &pack) ALWAYSINLINE {
                       // load rope
                       pack_rope_t rope;
                       if (laneId < LANES_PER_HEAD) {
                         // freq = load(reinterpret_cast<pack_freq_t *>(&freqs_cis[(warpId * WARP_M
                         // + rowId) * HEAD_DIM * 2 + laneId * PACK_SIZE * 2]));
...
#pragma unroll
                       ...
                       for (int i = 0; i < PACK_SIZE; i += 2) {
                         ...
                         // printf("block.x=%d block.y=%d warpId=%d rowId=%d (%d) freqs = %f %f %f
                         // %f\n",
                         //     blockIdx.x, blockIdx.y, warpId, rowId,
                         //     blockIdx.x * BLOCK_M + warpId * WARP_M + rowId,
                         //     (float)freq[i].x, (float)freq[i].y, (float)freq[i+1].x,
                         //     (float)freq[i+1].y
                         // );
                         // __trap();
    ...
    __device__ __forceinline__ void operator()(const BlockInfo binfo, fpsum_warp fpsum, int M,
      ...
      if (is_q || is_k) {
        apply(fpsum, args.out + bm * BLOCK_M * args.actualN + bn * BLOCK_N, M, N, K,
        ...
        EpilogueDefault()(binfo, fpsum, M, N, K,
  ...
  struct EpilogueRMSNormRope {
    ...
    using rotemb_warp =
      std::array<packed_rotemb_t, WARP_M_TILES * WARP_N_ROTEMB_TILES>;  // 128 regs

    struct Arguments {
      // **packed** [M, HEAD_DIM] float => [M // 16, HEAD_DIM // 8, WARP_SIZE] of packed_rotemb_t
      // aka [M // BLOCK_M, NUM_WARPS, WARP_M_TILES, WARP_N_TILES // NUM_HEADS_PER_WARP * 2,
      // WARP_SIZE]
    ...
    __device__ __forceinline__ static void load_rmsnorm(const half_t *ptr_rmsnorm_weight,
                                                        half_t *shmem) {
      const int laneId = threadIdx.x % WARP_SIZE;
      ...
      packed_t pack =
        load(reinterpret_cast<const packed_t *>(ptr_rmsnorm_weight + laneId * PACK_SIZE));
    ...
    __device__ __forceinline__ static void apply(fpsum_warp &fpsum,
                                                 const packed_rotemb_t *ptr_rotemb,
                                                 const half_t *ptr_rmsnorm_weight, float epsilon) {
      const int laneId = threadIdx.x % WARP_SIZE;
      const int warpId = threadIdx.x / WARP_SIZE;
...
#pragma unroll
        for (int n = 0; n < WARP_N_TILES_PER_HEAD; n++) {
          packed_f32psum_t rms =
            packed_fp16_to_fp32(load_rmsnorm_from_shmem(&shmem_rmsnorm[warpId][0], n));
#pragma unroll
          for (int m = 0; m < WARP_M_TILES; m++) {
  ...
  struct EpiloguePackQKV {
    ...
    __device__ __forceinline__ static void mask(packed_qkv_t &pack, uint32_t maskVal, int m,
                                                int maxRows) {
      const int laneId = threadIdx.x % WARP_SIZE;
    ...
    template <typename F>
    __device__ __forceinline__ static void apply(fpsum_warp &fpsum, packed_qkv_t *ptr_output,
                                                 int maxRows, F &&funcPack, attn_half2_t maskVal) {
      const int laneId = threadIdx.x % WARP_SIZE;
      const int warpId = threadIdx.x / WARP_SIZE;

      static_assert(HEAD_DIM == WARP_N);

      packed_qkv_t *ptrlane =
        &ptr_output[((warpId * WARP_M_TILES + 0) * WARP_N_TILES + 0) * WARP_SIZE + laneId];
    ...
    __device__ __forceinline__ void operator()(const BlockInfo binfo, fpsum_warp fpsum, int M,
      ...
      int block_offset =
        head_id * strideHead + bm * NUM_WARPS * WARP_M_TILES * WARP_N_TILES * WARP_SIZE;
  ...
  struct EpilogueLiteLA {
    __device__ __forceinline__ static packed_f32psum_t mma_litela(packed_fpsum_t k,
                                                                  packed_fpsum_t v,
    ...
    static constexpr int SHMEM_SIZE =
      NUM_WARPS * (LITELA_HEAD_DIM + 1) * (LITELA_HEAD_DIM + 8) * sizeof(float);

    // out_vk: [batch_size, num_heads, head_dim + 1, head_dim]
    __device__ __forceinline__ static void apply_litela(const BlockInfo binfo, fpsum_warp fpsum,
                                                        float *out_vk, int num_blocks_per_batch) {
      const int laneId = threadIdx.x % WARP_SIZE;
      const int warpId = threadIdx.x / WARP_SIZE;
      ...
      for (int head_id = 0; head_id < WARP_N / (LITELA_HEAD_DIM * 2); head_id++) {
        ...
        float *out_vk_current_head = out_vk + (batch_id * num_heads + global_head_id) *
                                                (LITELA_HEAD_DIM + 1) * LITELA_HEAD_DIM;
        ...
        for (int tile_v = 0; tile_v < LITELA_V_TILES; tile_v++) {
          for (int tile_k = 0; tile_k < LITELA_K_TILES; tile_k++) {
            ...
            for (int i = 0; i < WARP_M_TILES; i++) {
              packed_fpsum_t k =
                fpsum[i * WARP_N_TILES + head_id * (LITELA_HEAD_DIM * 2) / 16 + tile_k];
...
#pragma unroll
    ...
    __device__ __forceinline__ void operator()(const BlockInfo binfo, fpsum_warp fpsum, int M,
      ...
      if (bn < binfo.numBlocksN / 3) {
        ...
        return EpilogueDefault()(binfo, fpsum, M, N / 3, K,
                                 typename EpilogueDefault::Arguments{
                                   .out = args.out_q,
                                   .actualM = args.actualM,
                                   .actualN = N / 3,
    ...
    // each thread block mults BlockSize*HEAD_DIM q and (HEAD_DIM+1)*HEAD_DIM vk, in-place writes
    // back to q q:   [batch_size, #blocks, block_size, #heads, HEAD_DIM] vk:  [batch_size, #heads,
    // HEAD_DIM+1, HEAD_DIM]
    struct vk_mul_q_kernel {
      ...
      __device__ void operator()(half_t *q, const float *vk, float eps, int num_tokens) {
        ...
        half_t *localq =
          &q[(((batch_id * num_blocks + block_id) * block_size + threadIdx.x) * num_heads +
              head_id) *
             LITELA_HEAD_DIM];
        const float *localvk =
          &vk[(batch_id * num_heads + head_id) * (LITELA_HEAD_DIM + 1) * LITELA_HEAD_DIM];
        // half_t *localout = &out[(((batch_id * num_blocks + block_id) * block_size + threadIdx.x)
        // * num_heads
...
#pragma unroll
          for (int i = 0; i < LITELA_HEAD_DIM; i += sizeof(packed_vk) / sizeof(float)) {
            packed_vk vkpack =
              load(reinterpret_cast<const packed_vk *>(&localvk[j * LITELA_HEAD_DIM + i]));
#pragma unroll
  ...
  struct test_epilogue_kernel {
    ...
    __device__ __forceinline__ void operator()(Arguments args) {
      ...
      EpilogueDefault()(binfo, fpsum, args.M, args.N, 0,
                        typename EpilogueDefault::Arguments{
                          .out = args.output,
                          .actualM = args.actualM,
                          .actualN = args.actualN,
