// Copied from nunchaku/src/kernels/zgemm/mma_earlycuda.cuh
#pragma once
...
template <bool is_bf16>
__device__ __forceinline__ static uint4 mma_m16n8k16_f32f16f16f32(uint4 a, uint2 b,
                                                                  uint4 c) = delete;
...
__device__ __forceinline__ uint4 mma_m16n8k16_f32f16f16f32<true>(uint4 a, uint2 b, uint4 c) {
  ...
  asm volatile(
    ...
    : "r"(a.x), "r"(a.y), "r"(a.z), "r"(a.w), "r"(b.x), "r"(b.y), "r"(c.x), "r"(c.y), "r"(c.z),
      "r"(c.w));
  return d;
}
#endif
...
template <>
__device__ __forceinline__ uint4 mma_m16n8kx_s32common<mma_helper::s4, mma_helper::s4>(uint4 a,
                                                                                       uint2 b,
                                                                                       uint4 c) {
  uint4 d;
...
template <>
__device__ __forceinline__ uint4 mma_m16n8kx_s32common<mma_helper::u4, mma_helper::s4>(uint4 a,
                                                                                       uint2 b,
                                                                                       uint4 c) {
  uint4 d;
  static constexpr int K = 64;
