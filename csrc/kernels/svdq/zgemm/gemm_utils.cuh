// Copied from nunchaku/src/kernels/zgemm/gemm_utils.cuh
#pragma once
...
__device__ __forceinline__ static void store(T *addr, T val) {
  if constexpr (shmem) {
    if constexpr (sizeof(T) == 8) {
      ...
      asm volatile("st.shared.v2.b32 [%0], {%1, %2};" ::"l"(__cvta_generic_to_shared(addr)),
                   "r"(data.x), "r"(data.y));
      return;
    }
    if constexpr (sizeof(T) == 16) {
...
__device__ __forceinline__ static void store_pred(T *addr, T val, bool pred) {
  ...
  if constexpr (sizeof(T) == 16) {
    ...
    asm volatile(
      ...
      "}" ::"r"((int)pred),
      "l"(addr), "r"(data.x), "r"(data.y), "r"(data.z), "r"(data.w));
    return;
  }
