// Copied from nunchaku/src/kernels/zgemm/lora.cuh
#pragma once
...
class Lora<GEMMConfig_W4A4_FP16> : public GEMMBase<GEMMConfig_W4A4_FP16> {
 ...
 public:
  ...
  __device__ __forceinline__ static void load_lora_wgt(const packed_fpsum_t *ptr, int rtile,
                                                       int rank, lora_wgt_warp &result, bool pred) {
  ...
  // lora_act: [M / BLOCK_M, rank / WARP_R, NUM_WARPS, LORA_M_TILES, LORA_R_TILES, 8, WARP_SIZE] of
  // float
  __device__ __forceinline__ static void load_lora_act(const float *ptr, int rtile,
                                                       lora_act_warp &result, bool pred) {
    const int laneId = threadIdx.x % WARP_SIZE;
  ...
  __device__ __forceinline__ static void reduce_lora_act(float *ptr, int rtile, lora_act_warp val,
                                                         bool pred) {
    const int laneId = threadIdx.x % WARP_SIZE;
    const int warpId = threadIdx.x / WARP_SIZE;
  ...
  struct EpilogueLoraUp {
    ...
    __device__ __forceinline__ static void apply_lora_up(fpsum_warp &fpsum, const float *act,
                                                         const packed_fpsum_t *wgt,
                                                         const scale_t &scales, int rank,
                                                         bool alwaysfalse) {
...
#pragma unroll
      ...
      auto compute = [&scales](lora_act_warp A, lora_wgt_warp W, f32psum_warp &f32psum,
                               int rtile) ALWAYSINLINE {
        ...
        for (int m = 0; m < LORA_M_TILES; m++) {
          for (int n = 0; n < LORA_N_TILES; n++) {
            for (int r = 0; r < LORA_R_TILES; r++) {
              ...
              f32psum[m * WARP_N_TILES + n] =
                mma_f16xf16_f32(A_fp16[m * LORA_R_TILES + r], W[n * LORA_R_TILES + r],
                                f32psum[m * WARP_N_TILES + n]);
            }
          }
        }
...
#pragma unroll
    ...
    __device__ __forceinline__ void operator()(const BlockInfo binfo, fpsum_warp &fpsum, int M,
      ...
      apply_lora_up(fpsum,
                    args.lora_act + bm * (args.rank / WARP_R) *
                                      (NUM_WARPS * LORA_M_TILES * LORA_R_TILES * 8 * WARP_SIZE),
                    args.lora_wgt_up + bn * (BLOCK_N / 16) * (args.rank / 16) * WARP_SIZE,
                    args.scales, args.rank, args.alwaysfalse);
  ...
  struct EpilogueLoraDown {
    ...
    __device__ __forceinline__ static void apply_lora_down(fpsum_warp &fpsum, float *act,
                                                           const packed_fpsum_t *wgt, int rank,
                                                           bool alwaysfalse) {
      constexpr int NUM_STAGES = 2;
