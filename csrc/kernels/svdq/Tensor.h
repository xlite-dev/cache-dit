// Copied from nunchaku/src/Tensor.h
#pragma once
...
struct Device {
  ...
  static constexpr Device cpu(int idx = 0) { return Device{CPU, idx}; }
  static constexpr Device cuda(int idx = 0) { return Device{CUDA, idx}; }
};
...
class Buffer : public std::enable_shared_from_this<Buffer> {
 public:
  ...
  virtual bool isAsyncBuffer() { return false; }
...
class BufferMalloc : public Buffer {
 public:
  ...
  virtual ~BufferMalloc() { free(this->ptr); }
};
...
class BufferHost : public Buffer {
 public:
  ...
  virtual ~BufferHost() { checkCUDA(cudaFreeHost(this->ptr)); }
};
...
class BufferCUDA : public Buffer {
 public:
  ...
  virtual bool isAsyncBuffer() override { return true; }
};
...
class BufferCUDASync : public Buffer {
 public:
  ...
  virtual ~BufferCUDASync() { checkCUDA(cudaFree(this->ptr)); }
};
...
struct TensorShape {
  ...
  int ndims() const { return dataExtent.size(); }
...
class Tensor {
 ...
 public:
  ...
  Tensor operator[](int idx) const {
    ...
    Tensor result;
    result.shape =
      std::vector<int>(this->shape.dataExtent.begin() + 1, this->shape.dataExtent.end());
  ...
  Tensor reshape(TensorShape shape) const { return view(shape); }
  ...
  Tensor &zero_() {
    assert(this->is_contiguous());
    checkCUDA(cudaMemsetAsync(data_ptr<char>() + shape.offset * scalar_size(), 0,
                              shape.size() * scalar_size(), getCurrentCUDAStream()));
    return *this;
  }
  Tensor &copy_(Tensor other) {
    ...
    checkCUDA(cudaMemcpyAsync(data_ptr<char>(), other.data_ptr<char>(),
  ...
  // NOT IMPLEMENTED!!! DONT USE
  Tensor index(std::vector<std::any> whatever) { throw std::runtime_error("Not implemented"); }

 public:
  static Tensor allocate(TensorShape shape, ScalarType scalarType, Device device,
                         bool fill = false) {
    Tensor result;
    ...
    if (fill) {
      if (device.type == Device::CPU) {
        ...
        CUDADeviceContext ctx(device.idx);
        checkCUDA(cudaMemsetAsync(result.buffer->getPtr(), 0xCC, result.buffer->getSize(),
  ...
  static Tensor ones(TensorShape shape, ScalarType scalarType, Device device) {
    ...
    // FIXME FIXME FIXME
    checkCUDA(cudaMemsetAsync(result.buffer->getPtr(), 1, result.buffer->getSize(),
                              getCurrentCUDAStream()));
    return result;
  }
  static Tensor allocate_view(TensorShape shape, ScalarType scalarType,
                              std::shared_ptr<Buffer> buffer, size_t offset = 0) {
    Tensor result;
    result.buffer =
      std::make_shared<BufferView>(buffer, offset, shape.size() * scalarSize.at(scalarType));
    result.scalarType = scalarType;
    result.shape = shape;
    return result;
  }
 ...
 public:
  Tensor copy(Device device) const {
    ...
    // cudaMemcpyDefault, getCurrentCUDAStream())); if (this->device().type == Device::CPU &&
    // device.type == Device::CUDA) {
    //     checkCUDA(cudaMemcpyAsync(result.data_ptr(), this->data_ptr(), result.buffer->getSize(),
 ...
 public:
  // before launching an async operation, make sure to lock the buffer in case the buffer is freed
  // before GPU completes
  ...
  static void unlockBuffers() { lockedBuffers.clear(); }
  static void unlockBuffers(cudaStream_t stream) { lockedBuffers[stream].clear(); }
...
inline const std::map<Tensor::ScalarType, size_t> Tensor::scalarSize = {
  {INT8, 1}, {INT16, 2}, {INT32, 4},    {INT64, 8},    {FP16, 2},
  {FP32, 4}, {BF16, 2},  {FP8_E4M3, 1}, {FP8_E5M2, 1},
};
