// Copied from nunchaku/src/kernels/zgemm/gemm_base.cuh
#pragma once
...
class GEMMBase : public Config {
 public:
   ...
   *  ...
   *  230 231 238 239 246 247 254 255 <-- load by lane 31, broadcast to lane {3, 7, 11, ..., 31}
   * (8x)
   *
   ...
   * */
  static constexpr int WSCALES_PACK_SIZE =
    clamp(WARP_N / WARP_SIZE, 4 / sizeof(half), 16 / sizeof(half));
   ...
   */
  static constexpr int ASCALES_PACK_SIZE =
    clamp(WARP_M / WARP_SIZE, 4 / sizeof(half), 16 / sizeof(half));
  ...
  __device__ __forceinline__ static packed_f32psum_t mma_f16xf16_f32(packed_fpsum_t a,
                                                                     packed_fpsum_t b,
...
#pragma unroll
  ...
  // activation: row major, [M / BLOCK_M, K / WARP_K, NUM_WARPS, WARP_M_TILES, WARP_SIZE] of
  // packed_act_t
  __device__ __forceinline__ static void load_act(const packed_act_t *act, int k, int K,
...
#pragma unroll
    for (int i = 0; i < WARP_M_TILES; i++) {
      ...
      out[i] =
        load_pred(&act[((k * NUM_WARPS + warpId) * WARP_M_TILES + i) * WARP_SIZE + laneId], pred);
      //}
    }
  }

  // weight: column major: [N / BLOCK_N, 1, K / WARP_K, WARP_N_TILES, WARP_SIZE] of packed_wgt_t
  __device__ __forceinline__ static void load_wgt(const packed_wgt_t *wgt, int k, int K,
                                                  wgt_warp &out, bool pred) {
    int laneId = threadIdx.x % WARP_SIZE;
...
#pragma unroll
  ...
  // ascales: row major [M / BLOCK_M, K / group size, NUM_WARPS, ASCALES_NUM_PACKS,
  // ASCALES_VALID_LANES] of packed_ascale_t
  __device__ __forceinline__ static void load_ascale(const packed_ascale_t *ascales, int group,
                                                     int M, ascale_warp &out, bool pred) {
    int laneId = threadIdx.x % WARP_SIZE;
    int warpId = threadIdx.x / WARP_SIZE;
#pragma unroll
    for (int i = 0; i < ASCALES_NUM_PACKS; i++) {
      // if (pred && laneId < ASCALES_VALID_LANES) {
      // out[i] = ascales[(group * M / WARP_M + warpId) * ASCALES_VALID_LANES * ASCALES_NUM_PACKS +
      // i * ASCALES_VALID_LANES + laneId];
      out[i] =
        load_pred(&ascales[(group * NUM_WARPS + warpId) * ASCALES_NUM_PACKS * ASCALES_VALID_LANES +
                           i * ASCALES_VALID_LANES + laneId],
  ...
  // wscales: column major [N / BLOCK_N, K / group size, 1, WSCALES_NUM_PACKS, WSCALES_VALID_LANES]
  // of packed_wscale_t
  // </del>
  __device__ __forceinline__ static void load_wscale(const packed_wscale_t *wscales, int group,
                                                     int N, wscale_warp &out, bool pred) {
    int laneId = threadIdx.x % WARP_SIZE;

    // static_assert(WSCALES_NUM_PACKS * WSCALES_VALID_LANES == 32);
    // static_assert(sizeof(packed_wscale_t) == 8);

    // const packed_wscale_t *ptr = &wscales[(group * WSCALES_NUM_PACKS + 0) * WSCALES_VALID_LANES +
    // laneId];
...
#pragma unroll
    for (int i = 0; i < WSCALES_NUM_PACKS; i++) {
      ...
      // out[i] = wscales[group * N / WARP_N * WSCALES_VALID_LANES * WSCALES_NUM_PACKS + i *
      // WSCALES_VALID_LANES + laneId]; out[i] = load(&wscales[group * N / WARP_N *
      // WSCALES_VALID_LANES * WSCALES_NUM_PACKS + i * WSCALES_VALID_LANES + laneId]);
  ...
  __device__ __forceinline__ static void apply_scales(F &&getpsum, ascale_warp ascale,
    ...
    for (int j = 0; j < WARP_N_TILES; j++) {
      ...
      for (int i = 0; i < WARP_M_TILES; i++) {
        ...
        //     printf("before ws2 = %f %f fsum.data[%d] = %f %f\n", (float)ws2.x, (float)ws2.y,
        //     target, (float)fsum.data[target].x, (float)fsum.data[target].y);
        // }

        fsum.data[0] =
          __hfma2(i2f::int2half2(psum.data[0], psum.data[1]), __hmul2(asx[i], ws1), fsum.data[0]);
        fsum.data[1] =
          __hfma2(i2f::int2half2(psum.data[2], psum.data[3]), __hmul2(asy[i], ws1), fsum.data[1]);
        fsum.data[2] =
          __hfma2(i2f::int2half2(psum.data[4], psum.data[5]), __hmul2(asx[i], ws2), fsum.data[2]);
        fsum.data[3] =
          __hfma2(i2f::int2half2(psum.data[6], psum.data[7]), __hmul2(asy[i], ws2), fsum.data[3]);
   ...
   */
  __device__ __forceinline__ static void pack_ascales(const half_t *input,
                                                      packed_ascale_t *output) {
    const int laneId = threadIdx.x % WARP_SIZE;
#pragma unroll
...
#pragma unroll
        for (int i = 0; i < ASCALES_PACK_SIZE; i += 2) {
          tmp.data[i / 2].x = input[j * ASCALES_PACK_SIZE * WARP_SIZE +
                                    laneId / 8 * 8 * ASCALES_PACK_SIZE + laneId % 8 + i * 8];
          tmp.data[i / 2].y = input[j * ASCALES_PACK_SIZE * WARP_SIZE +
                                    laneId / 8 * 8 * ASCALES_PACK_SIZE + laneId % 8 + (i + 1) * 8];
        }
   ...
   */
  __device__ __forceinline__ static void pack_wscales(const half_t *input,
                                                      packed_wscale_t *output) {
    const int laneId = threadIdx.x % WARP_SIZE;
#pragma unroll
...
#pragma unroll
        for (int i = 0; i < WSCALES_PACK_SIZE; i += 2) {
          tmp.data[i / 2] = *reinterpret_cast<const half2_t *>(
            &input[j * WSCALES_PACK_SIZE * WARP_SIZE + laneId / 4 * 4 * WSCALES_PACK_SIZE +
                   laneId % 4 * 2 + i * 4]);
        }
  ...
  struct unpack_fpsum {
    ...
    template <typename... F>
    __device__ __forceinline__ void operator()(fpsum_warp fpsum, half_t *output, int stride,
                                               int maxRows, int maxCols, void *shmem,
                                               F &&...plugins) {
      const int laneId = threadIdx.x % WARP_SIZE;
      ...
      //     // reduce_tmp = load<true>(reinterpret_cast<pack_t *>(&reduce_result[laneId *
      //     PACK_SIZE]));
      // }
...
#pragma unroll
        for (int row = 0; row < 8; row++) {
          ...
          // if (pred) {
          store_pred(
            reinterpret_cast<pack_t *>(&output[(i * INSN_M + row) * stride + laneId * PACK_SIZE]),
            pack, pred);
          // }
        }
        __syncwarp();
...
#pragma unroll
        for (int row = 0; row < 8; row++) {
          ...
          // if (pred) {
          store_pred(reinterpret_cast<pack_t *>(
                       &output[(i * INSN_M + 8 + row) * stride + laneId * PACK_SIZE]),
                     pack, pred);
          // }
        }
        __syncwarp();
      ...
      // if (enableReduce) {
      //     store<true>(reinterpret_cast<pack_t *>(&reduce_result[laneId * PACK_SIZE]),
      //     reduce_tmp);
      // }
    }
  };
  ...
  struct load_act_to_fpsum {
    ...
    __device__ __forceinline__ void operator()(const half_t *input, int stride, int maxRows,
                                               int maxCols, fpsum_warp &out, void *shmem) {
      const int laneId = threadIdx.x % WARP_SIZE;
...
#pragma unroll
        for (int row = 0; row < INSN_M; row++) {
          ...
          if constexpr (fuse_glu) {
            ...
            if (pred) {
              raw = load(reinterpret_cast<const packed_raw_input *>(
...
#pragma unroll
            ...
            if (pred) {
              pack = load(reinterpret_cast<const packed_input *>(
  ...
  struct EpilogueDefault {
    ...
    __device__ __forceinline__ void operator()(const BlockInfo binfo, fpsum_warp fpsum, int M,
      ...
      __shared__ alignas(128)
        uint8_t shmem[NUM_WARPS][ceilDiv(unpack_fpsum::SHMEM_SIZE, 128) * 128];

      const int m_offset = binfo.bm * BLOCK_M + warpId * WARP_M;
      const int n_offset = binfo.bn * BLOCK_N;

      unpack_fpsum()(fpsum, args.out + m_offset * args.actualN + n_offset, args.actualN,
                     args.actualM - m_offset, args.actualN - n_offset, shmem[warpId],
  ...
  struct EpilogueBias {
    struct Arguments {
      const packed_wscale_t
        *bias;  // [N / BLOCK_N, WSCALES_NUM_PACKS, WSCALES_VALID_LANES] of packed_wscale_t
      const packed_wscale_t *scale;
    };

    __device__ __forceinline__ void apply_bias(fpsum_warp &fpsum, int M, int N, int K,
