...
def _import_safetensors():
    try:
        ...
        raise ImportError(
            ...
            "Install with `pip install cache-dit[quantization]` or `pip install safetensors`."
        ) from exc
...
class SVDQPTQContext:
    ...
    def _collect_candidate_layer_names(self) -> list[str]:
        ...
        if not candidate_layer_names:
            raise ValueError(
                "SVDQ PTQ found no quantizable Linear layers in the target module. "
...
def _validate_quant_config_snapshot(path: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("version") != _SVDQ_QUANT_CONFIG_VERSION:
        raise ValueError(
            f"Unsupported SVDQ quant_config version {payload.get('version')} in {path}."
        )
    ...
    if not isinstance(exclude_layers, list) or any(
        not isinstance(layer_name, str) for layer_name in exclude_layers
    ):
    ...
    if repeated_blocks is not None and (
        not isinstance(repeated_blocks, list)
        or any(not isinstance(block_name, str) for block_name in repeated_blocks)
    ):
...
def _resolve_directory_load_source(directory_path: str) -> _ResolvedSVDQLoadSource:
    ...
    resolved_checkpoint_path = (
        ...
        else os.path.normpath(os.path.join(normalized_directory_path, checkpoint_path))
    )
    if not os.path.isfile(resolved_checkpoint_path):
        raise FileNotFoundError(
            "SVDQ PTQ checkpoint referenced by quant_config.json not found: "
...
def _resolve_load_path(
    ...
    raise TypeError(
        "quantize_config_or_path must be a QuantizeConfig or a safetensors file path, "
...
def _validate_serialized_payload(path: str, payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("format") != "cache_dit_svdq_ptq":
        raise ValueError(
            f"Invalid SVDQ PTQ checkpoint format in {path}: {payload.get('format')!r}."
        )
    if payload.get("version") != _SVDQ_FORMAT_VERSION:
        raise ValueError(
            f"Unsupported SVDQ PTQ checkpoint version {payload.get('version')} in {path}."
        )
...
def quantize_svdq_ptq(module: nn.Module, quantize_config: QuantizeConfig) -> nn.Module:
    ...
    if check_quantized(module):
        logger.warning(
            "Module %s is already quantized, skipping SVDQ PTQ.", module.__class__.__name__
        )
        return module
    ...
    if not quantized_layer_names:
        raise RuntimeError(
            "SVDQ PTQ completed calibration but no layers were quantized. "
...
def load_svdq(
    ...
    if quant_config_snapshot is not None and quant_config_snapshot["quant_type"] != quant_type:
        raise ValueError(
            "The SVDQ checkpoint quant_type does not match quant_config.json: "
    ...
    if isinstance(quantize_config_or_path, QuantizeConfig):
        ...
        if quantize_config_or_path.quant_type != quant_type:
            raise ValueError(
                "The SVDQ checkpoint quant_type does not match QuantizeConfig: "
    ...
    for layer_name in quantized_layer_names:
        ...
        layer_state_dict = {
            ...
            if key.startswith(f"{serialized_name}.")
        }
    ...
    _attach_quantization_metadata(
        ...
        exclude_layers=(
            list(quantize_config_or_path.exclude_layers or [])
            if isinstance(quantize_config_or_path, QuantizeConfig)
            else list(
                (quant_config_snapshot or {}).get(
        ...
        checkpoint_path=checkpoint_path,
        quantize_config=(
            quantize_config_or_path if isinstance(quantize_config_or_path, QuantizeConfig) else None
        ),
    )
    module.train(was_training)
    _log_load_summary(
        ...
        checkpoint_path=checkpoint_path,
        svdq_kwargs=svdq_kwargs,
        verbose=isinstance(quantize_config_or_path, QuantizeConfig)
        and quantize_config_or_path.verbose,
    )
    return module
