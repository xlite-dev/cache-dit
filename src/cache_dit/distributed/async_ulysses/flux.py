...
__all__ = ["FluxAsyncUlyssesPlanner"]


@AsyncUlyssesRegistry.register("FluxTransformer2DModel")
class FluxAsyncUlyssesPlanner(AsyncUlyssesPlanner):

  @classmethod
  def get_method_patches(cls) -> Tuple[MethodPatchSpec, ...]:
    return (
      MethodPatchSpec(FluxAttnProcessor, "__call__", cls._build_flux_attn_patch),
      MethodPatchSpec(FluxSingleTransformerBlock, "forward", cls._build_flux_single_block_patch),
    )
  ...
  def _async_ulysses_attn_flux(
    ...
    cp_config = require_cp_config(self, "FluxAttnProcessor")
  ...
  @staticmethod
  def _build_flux_attn_patch(original):
  ...
  @staticmethod
  def _build_flux_single_block_patch(original):
